    ...
    children:[
      {
        path:'index',
        name:'我的学习',
        component: () => import('@/views/stu/my-learning'),
        meta: { title: '我的学习', icon: 'table' }
      },
      {
        path: 'submit',
        name: '提交作业',
        component: () => import('@/views/stu/submit'),  // 引入提交作业页面
...
// 动态添加路由
export function addUserRoutes(accountType) {
  let routes=[];
  if (hasRoutesAdded) return;
  if(accountType === 1){
    routes=userRoutes;
